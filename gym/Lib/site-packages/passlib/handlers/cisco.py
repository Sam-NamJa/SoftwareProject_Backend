...
class cisco_pix(uh.HasUserContext, uh.StaticHandler):
    ...
    :param str user:
        String containing name of user accounts this password is associated with.

        This is *required* in order to correctly hash passwords associated
        with a user accounts on the Cisco device, as it is used to salt
        the hash.

        Conversely, this *must* be omitted or set to ``""`` in order to correctly
        hash passwords which don't have an associated user accounts
    ...
    def _calc_checksum(self, secret):
        ...
        # setting 32-char password for a user accounts,
