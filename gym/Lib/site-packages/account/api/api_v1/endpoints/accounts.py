...
def get_account_for_user(
    *,
    ...
    """
    Retrieve accounts for a logged in user.
    """
...
def create_account(
    *,
    ...
    if account:
        raise HTTPException(
            status_code=409, detail="An accounts with this doc_id already exists",
        )
...
def update_account(
    *,
    ...
    # If user is an accounts admin, check ensure they update their own accounts.
...
def retrieve_users_for_account(
    *,
    ...
    """
    Retrieve users for an accounts.
    """
    account = crud.account.get(db, id=account_id)
    if not account:
        raise HTTPException(
...
def retrieve_users_for_own_account(
    *,
    ...
    """
    Retrieve users for own accounts.
    """
    account = crud.account.get(db, id=current_user.account_id)
    if not account:
        raise HTTPException(
