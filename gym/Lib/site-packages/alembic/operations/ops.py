    ...
    def create_table(
        ...
        :meth:`.Operations.bulk_insert`::
            ...
            op.bulk_insert(
                account_table,
                [
                    {"name": "A1", "description": "accounts 1"},
                    {"name": "A2", "description": "accounts 2"},
                ]
            )
...
class ExecuteSQLOp(MigrateOperation):
    ...
    def execute(
        ...
        below where we use an update construct::
            ...
            op.execute(
                accounts.update().\\
                    where(accounts.c.name==op.inline_literal('accounts 1')).\\
                    values({'name':op.inline_literal('accounts 2')})
                    )
        ...
        also be used normally, use the "bind" available from the context::
            ...
            connection.execute(
                accounts.update().where(accounts.c.name=='accounts 1').
                values({"name": "accounts 2"})
            )
