...
``SavingsAccount`` will be present::
    ...
    FROM "user" JOIN accounts ON "user".id = accounts.user_id
...
would use an outer join::
    ...
    SELECT "user".id AS user_id, "user".name AS user_name,
    accounts.balance AS account_balance
    FROM "user" LEFT OUTER JOIN accounts ON "user".id = accounts.user_id
    WHERE accounts.balance <  :balance_1 OR accounts.balance IS NULL
...
a correlated SELECT::
    ...
    SELECT "user".id AS user_id, "user".name AS user_name
    FROM "user"
    WHERE (SELECT sum(accounts.balance) AS sum_1
    FROM accounts
    WHERE accounts.user_id = "user".id) > :param_1
